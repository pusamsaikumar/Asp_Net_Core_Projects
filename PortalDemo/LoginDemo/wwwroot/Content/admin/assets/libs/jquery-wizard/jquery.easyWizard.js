...
(function( $ ) {
	...
	var easyWizardMethods = {
			...
			return this.each(function() {
				...
				if(thisSettings.steps > 1) {
					...
					$steps.each(function(index) {
                        ...
                        //click action to header
						stepText = thisSettings.stepsText.replace('{n}', '<span class="wizard-header-num">' + step + '</span>');
						stepText = stepText.replace('{t}', "<a href='javascript:void(0);' rel='" + step + "' class='wizard-header-link'>" + $(this).attr('data-step-title') + "</a>");
                    ...
                    //Added this to header click
				    // When we click on wizard-header-link
					$('a.wizard-header-link').click(function () {

					    //Updated code by adding if condition on 27/10/2016 for preventing wrapper issue when double clicked on data-step title 
					    if (!($(this).closest("li").hasClass("current"))) {

					        //Previous code
					        $wizard = $(this).parents('.easyWizardElement');
					        easyWizardMethods.goToStep.call($wizard, this.rel);
		...
		resize: function (activeStepObj) {
		    ...
		    /// Added resize event on November 2, 2016 for wrapper resizing

		    if (activeStepObj != undefined) {
		        // gets new width
		        $this = $(this);
		        thisSettings.width = $(this).width();
		        ...
		        $this.find('.easyWizardWrapper').css({
		            'margin-left': thisSettings.width * (activeStepObj - 1) * -1
		        });
